class AppEnviroment {
  // static const String URL = '192.168.1.79:3000';
  static const String URL = 'senia.ga:3000';

  
  static const String URL_PREENDPOINT = '/api/v1';
  static const String URL_ENDPOINT_AUTH = 'auth';
  static const String URL_ENDPOINT_USER = 'user';
  static const String URL_ENDPOINT_LETTER = 'letter';


  static const List<String> HANDS_LETTER = ["Izquierda", "Derecha"];
  static const List<String> TYPES_LETTERS = ["Entrenamiento", "Validación"];
  static const List<String> LETTERS = ["A","B","C","D","E","F","G","H","I","J","K","L","LL","M","N","Ñ","O","P","Q","R","RR","S","T","U","V","W","X","Y","Z"];
}
